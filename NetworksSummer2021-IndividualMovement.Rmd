...
turn_choices_by_ant_trunk <- turn_choices %>% 
  ...
  summarize(num_turns=n(), 
            ...
            total_trunk_turns=turns_to_trunk+turns_away_from_trunk) %>%
  ungroup()

trunk.model <- lm(turns_to_trunk ~ total_trunk_turns + 0,
                  data=turn_choices_by_ant_trunk)
no.bias.model <- lm(turns_to_trunk ~ offset(0.5*total_trunk_turns)+0, 
                    data=turn_choices_by_ant_trunk)
summary(trunk.model)
anova(trunk.model,no.bias.model)
```
Like when analyzing left vs right or wide vs narrow turns the ants made, the no bias model is that the ants have a 50:50 chance of picking either option between "to trunk" or "away from trunk". It appears that ants turn toward the trunk about 37.920% of the time overall.
...
anova(trunk.model,no.bias.model)
```
Before inspecting, the ants have at 36.336% chance of turning toward the trunk. After inspecting, that chance is 38.977%. I could do a statistical test to determine whether this difference in proportions is significant, but that's not the focus here. I could also distinguish between number of inspections done and do ANOVA based on that, but I'm not sure whether that's useful to know or not.
