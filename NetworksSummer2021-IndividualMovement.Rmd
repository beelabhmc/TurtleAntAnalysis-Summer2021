...
```{r}
#Need to consider all possible turn choices
#use lag/lead to mutate new var
#would probably be better to make a 3x3 table
turn_choices_seq <- turn_choices %>%
  group_by(Colony,Ant.ID) %>%
  filter(Num.Inspections==0) %>%
  filter(Ant.In.Pair=="A") %>% 
  mutate(Next.Turn.Type = lead(Turn.Type)) %>%
  #filter(Next.Turn.Type != NA) %>% # want to remove the turns for which Next.Turn.Type == NA
  ungroup()

#View(turn_choices_seq %>% select(Date, Colony, Ant.ID, Turn.Type, Next.Turn.Type))
Turn.Seq.Pair <- c(rep(c("test1", "test2"), times=length(turn_choices_seq$Turn.Type)))
for (i in length(turn_choices_seq$Turn.Type)) {
  Turn.Seq.Pair[i] <- c(turn_choices_seq$Turn.Type[i], turn_choices_seq$Next.Turn.Type[i])
}
```
