...
junction_type is a table of junction ID number and handedness where left is 1, right is -1, and the two edge cases where both exits are narrow are 0. These could be excluded from our model but they do have a handedness, which cannot be extracted by analyzing narrow/wideness.

Now I implement these variables into turns as Direction.From and Junction.Handedness.
...
turns <- turns %>%
  mutate(
    Turn.Angle = case_when(
      Direction.From=='main' & Turn.Type=='L' & Junction.Handedness==1 ~ "sharp",
      Direction.From=='main' & Turn.Type=='R' & Junction.Handedness==1 ~ "shallow",
      Direction.From=='main' & Turn.Type=='L' & Junction.Handedness==-1 ~ "shallow",
      Direction.From=='main' & Turn.Type=='R' & Junction.Handedness==-1 ~ "sharp",
  ...
  mutate(
    isSharpTurn = if_else(
      Turn.Angle == "sharp",TRUE,FALSE
    ),
    isLeft = case_when(
      Junction.Handedness == 1 ~ TRUE,
      Junction.Handedness == -1 ~ FALSE
    ),
    Angle.From = case_when(
      Direction.From == 'main' ~ "main",
      Direction.From == 'left' & Junction.Handedness == 1 ~ "sharp",
      Direction.From == 'left' & Junction.Handedness == -1 ~ "shallow",
      Direction.From == 'right' & Junction.Handedness == 1 ~ "shallow",
      Direction.From == 'right' & Junction.Handedness == -1 ~ "sharp",
...
Finally I left_join the turn choices by Node.From and Node.To to get a column for angle of turn, with U turns removed (they are not useful in this analysis).

```{r}
turn_choices_angle <- turn_choices %>%
  left_join(select(turns, Node.From,Node.To,Turn.Angle,isLeft,Angle.From,isSharpTurn), by=c("Node.From", "Node.To")) %>%
  filter(Turn.Type != "U")
```

Now the table turn_choices_angle feeds data to perform binomial glm regression and find the data from this.

```{r}
angle_model_plus <- glm(isSharpTurn ~ Angle.From + isLeft, family = binomial, turn_choices_angle)
angle_model_star <- glm(isSharpTurn ~ Angle.From * isLeft, family = binomial, turn_choices_angle)
```
