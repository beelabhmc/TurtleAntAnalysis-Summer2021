...
turn_choices_by_ant_width <- turn_choices %>% 
  ...
  summarize(num_turns=n(), 
            ...
            total_turns=narrow_turns+wide_turns) %>%
  ungroup()

wide.model <- lm(wide_turns ~ total_turns + 0,
                  data=turn_choices_by_ant_width)
no.bias.model <- lm(wide_turns ~ offset(0.5*total_turns)+0, 
                    data=turn_choices_by_ant_width)
summary(wide.model)
anova(wide.model,no.bias.model)
```

Comparing the fitted model with a model of known slope (=0.5) shows that outgoing ants have a significant wide bias, staying on the wide branch 55.7% of the time where width could be considered. This plot illustrates this:

```{r}
max_turns <- max(turn_choices_by_ant_width$total_turns)
ggplot(turn_choices_by_ant_width, aes(x=total_turns,y=wide_turns)) + 
  geom_point(aes(color=Colony)) + 
  geom_smooth(method="lm", formula=y ~ x + 0, color="black") +
  geom_abline(slope=0.5,intercept=0,lty=2) + 
