...
We can use this model to generate predictions for all turns, based just on the direction of approach and which side has the sharper turn. To do this, we need to join the table containing turns for all junctions (`turns`) with the table containing the predictions based on these factors (`approach.types`).

```{r}
turn_predictions <- left_join(turns, approach.types)
turn_predictions <- turn_predictions %>%
  rename(Sharp.Turn.Probability = Predictions) %>%
  mutate(Turn.Probability = case_when(isSharpTurn=="TRUE" ~
                                        Sharp.Turn.Probability,
                                      isSharpTurn=="FALSE" ~
                                        1-Sharp.Turn.Probability))
...
```{r}
approach.types <- approach.types
fixed_effects <- as_tibble(fixed_effects) %>% mutate(sharpIsLeft = as.logical(sharpIsLeft))
approach.types
fixed_effects
...
testribbon <- tibble(Junction.Handedness = c(rep(c(rep("LH",6),rep("RH",6)),1)),
                     Angle.From= c(rep(c(rep("Main",2), rep("Primary",2), rep("Secondary",2)),2)),
                     x = c(rep(c(0,maxtotal),6)),
                     ymax = c(0,5,0,10,0,15,0,20,0,15,0,10),
                     ymin = c(rep(c(0, 1),6))
                     )
testpolygon <- tibble(xu = c(0,0,maxtotal, maxtotal),
                      yu = c(0,maxsharp,maxsharp,maxtotal/2),
                      xl = c(0,0,maxtotal,maxtotal),
                      yl = c(0,0,0,maxtotal/2))
...
angle_ribbon_data_x <- angle_ribbon_data_pre %>%
  ...
  select(Junction.Handedness, Angle.From, x)
...
Testing zone for figure:

```{r}
angle_graph_data %>%
  ggplot() +
  facet_grid(cols = vars(Junction.Handedness), rows = vars(Angle.From)) +
  #geom_abline(slope=0.5,color = "black", intercept=0, size=1) +
  geom_point(mapping = aes(x = Total, y = Sharp)) +
  geom_abline(data=angle_fit_data, aes(slope = fit, intercept = 0),size = 1) +
  #geom_abline(data=angle_fit_data, aes(slope = upper, intercept = 0), lty = 2, size = .5) +
  #geom_abline(data=angle_fit_data, aes(slope = lower, intercept = 0), lty = 2, size = .5) +
  geom_ribbon(data=angle_ribbon_data2, aes(x = x, ymin = ymin, ymax = ymax), alpha = .3) +
  geom_polygon(data = testpolygon2, aes(x = xu, y = yu), alpha = .25, fill = "red") +
  geom_polygon(data = testpolygon2, aes(x = xl, y = yl), alpha = .25, fill = "blue") +
  #geom_smooth(data = angle_graph_data, x = Total, y = Sharp, method="lm", color="black") +
  theme_classic(base_size = 15) +
  coord_cartesian(xlim = c(0,maxtrue), ylim = c(0, maxtrue/2)) +
  labs(y = "Number of Sharp Turns", x = "Number of Total Turns")
ggsave("images/Angle_Data_Visualization_Test.png",dpi=300, width = 5, height = 7, units = "in")
```

Defunct log data figure:

```{r}
angle_graph_data %>%
  ggplot() +
  geom_point(mapping = aes(x = LogTotal, y = LogSharp, color = Level)) +
  geom_abline(slope=1,intercept=log(.5),color="blue") +
  geom_abline(data=angle_fit_data, aes(slope = 2*fit, intercept = log(.5)), lty=2) +
  geom_abline(data=angle_fit_data, aes(slope = 2*upper, intercept = log(.5)), lty = 3) +
  geom_abline(data=angle_fit_data, aes(slope = 2*lower, intercept = log(.5)), lty = 3) +
  facet_grid(rows = vars(Junction.Handedness), cols = vars(Direction.From)) +
  theme_minimal(base_size = 20) +
  labs(title = "Log Sharp Turns vs Log Total Turns", y = "Log Number of Sharp Turns", x = "Log Number of Total Turns")
ggsave("images/Log_Angle_Data_Visualization.png")
```
