...
angle_graph_data %>%
  ...
  geom_abline(slope=0.5,intercept=0,lty=2) +
  geom_abline(data=angle_fit_data, slope=fit, intercept=0, color="blue")# +
  #geom_ribbon(data= angle_fit_data, aes(x= Direction.From, ymin= lower, ymax= upper), alpha= 0.3, fill="blue") +
  theme_minimal(base_size = 25)
```
...
angle_graph_data %>%
  ggplot() +
  geom_point(mapping = aes(x = Total, y = Sharp)) +
  geom_abline(slope=0.5,color = "blue", intercept=0) +
  geom_abline(data=angle_fit_data, aes(slope = fit, intercept = 0), lty=2) +
  geom_abline(data=angle_fit_data, aes(slope = upper, intercept = 0), lty = 3) +
  geom_abline(data=angle_fit_data, aes(slope = lower, intercept = 0), lty = 3) +
...
ggsave("images/Angle_Data_Visualization.png")
```
...
angle_graph_data %>%
  ggplot() +
  geom_point(mapping = aes(x = LogTotal, y = LogSharp)) +
  geom_abline(slope=1,intercept=log(.5),lty=2) +
  geom_abline(data=angle_fit_data, aes(slope = 2*fit, intercept = log(.5)), color="blue", lty=2) +
  geom_abline(data=angle_fit_data, aes(slope = 2*upper, intercept = log(.5)), color="blue", lty = 3) +
  geom_abline(data=angle_fit_data, aes(slope = 2*lower, intercept = log(.5)), color="blue", lty = 3) +
  facet_grid(rows = vars(Junction.Handedness), cols = vars(Direction.From)) +
